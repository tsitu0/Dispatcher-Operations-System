...
import { AlertCircle, Loader2, Edit, Trash2, Eye, Link2 } from "lucide-react"
    ...
    <Card>
      ...
      <CardContent>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">No containers match your search criteria</div>
        ) : (
              ...
              <tbody>
                {containers.map((container, index) => (
                  <tr key={container.id || `${container.containerNumber}-${index}`} className="border-b hover:bg-muted/50">
                    <td className="py-2.5 px-3 text-xs">{container.caseNumber || "-"}</td>
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex gap-2">
                        ...
                        <Link href={`/containers/${container.id}/edit`}>
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Edit container">
